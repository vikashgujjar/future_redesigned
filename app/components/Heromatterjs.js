...
// framer-motion (the drag physics behind each skill chip) is only used here.
// The chips are absolutely positioned inside an arena box with a fixed
// height, so deferring them client-side can't shift any surrounding layout —
// safe to keep off the initial bundle for every page that renders this
// section (the whole thing is also max-md:hidden, so mobile never needs it
// at all).
const Pill = dynamic(() => import("./SkillPill"), { ssr: false });
